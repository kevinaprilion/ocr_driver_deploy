...
// Cookies.set('time', d.toLocaleString());
// Cookies.set('detik', d.getSeconds());
// Cookies.set('menit', d.getMinutes());

function AbsentOut() {
    const theme = useTheme();
    // const [dateFrom, setDateFrom] = useState(datestringNow)
    ...
    const { handleSubmit, getFieldProps, errors, touched } = useFormik({
        initialValues: {
            dateCheckOut: datestringNow,
            timeCheckOut: time,
            noted: "",
            mileage: "",
        },
        // validationSchema: Yup.object({
        //     dateCheckOut: Yup.string()
        //         .required("Harus diisi!")
        //         .min(6, "Harus 6 karakter atau lebih!"),
        // }),
        onSubmit: (values) => {
            AttendRequest.checkOut(values, handleOpenModal);
        },
    });
    const body = (
                                    ...
                                    <form onSubmit={handleSubmit}>
                                        <div className="md:w-96 sm:w-80 py-3">
                                            ...
                                            <div className="font-sans text-abuabuSecondary text-xl font-medium sm:mt-1">
                                                <TextField type='date' style={{ width: "100%" }} className="form-control" name="dateCheckOut"
                                                    helperText={
                                                        touched.dateCheckOut ? errors.dateCheckOut : ""
                                                    }
                                                    error={
                                                        touched.dateCheckOut &&
                                                        Boolean(errors.dateCheckOut)
                                                    }
                                                    {...getFieldProps("dateCheckOut")}
                                            ...
                                            <div className="font-sans text-abuabuSecondary text-xl font-medium sm:mt-1">
                                                <TextField type='time' style={{ width: "100%" }} className="form-control"
                                                    name="timeCheckOut"
                                                    helperText={
                                                        touched.timeCheckOut ? errors.timeCheckOut : ""
                                                    }
                                                    error={
                                                        touched.timeCheckOut &&
                                                        Boolean(errors.timeCheckOut)
                                                    }
                                                    {...getFieldProps("timeCheckOut")}
